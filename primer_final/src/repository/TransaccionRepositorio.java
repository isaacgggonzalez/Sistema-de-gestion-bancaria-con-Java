...
public class TransaccionRepositorio {
    ...
    public Cliente obtenerCliente(long cedula) {
        ...
        try (Connection conexion = ConexionBD.conectar();
             PreparedStatement preparedStatement = conexion.prepareStatement(OBTENER_CLIENTE)) {
    ...
    public Cuenta obtenerCuenta(long numeroCuenta) {
        Cuenta cuenta = null;
        try (Connection conexion = ConexionBD.conectar();
             PreparedStatement preparedStatement = conexion.prepareStatement(OBTENER_CUENTA)) {
    ...
    public boolean verificarPinCuenta(long cedula, long numeroCuenta, int clave) {
        ...
        try (Connection conexion = ConexionBD.conectar()) {
            ...
            credencialesCorrectas = verificarCredencialesEnBaseDeDatos(cedula, numeroCuenta, clave);
        } catch (SQLException e) {
     ...
     */
    private boolean verificarCredencialesEnBaseDeDatos(long cedula, long numeroCuenta, int clave) throws SQLException {
    ...
    */
    ...
    public Boolean validarPinTransaccion(Long pinTransaccion){
        ...
        */
         public boolean confirmarDatos(long destinoTransferencia, long cedula, String nombreDestinatario) {
            Connection connection = ConexionBD.conectar();
            try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM cliente c, cuenta cu WHERE c.cedula = ? AND cu.numero_cuenta = ? AND c.nombre = ?")) {
                ...
                preparedStatement.setString(3, nombreDestinatario);
     ...
     */
    public void aumentarDeuda(long numeroTarjeta, double monto) {
        System.out.println("aumentar numero_tarjeta: "+numeroTarjeta);
        System.out.println("aumentar monto: "+monto);
        Connection connection = ConexionBD.conectar();
        try{
            PreparedStatement statement = connection.prepareStatement(AUMENTAR_DEUDA_TARJETA);
            statement.setDouble(1, monto);
            statement.setLong(2, numeroTarjeta);
            statement.executeUpdate();

        }catch (SQLException ex){
            Logger.getLogger(TransaccionRepositorio.class.getName()).log(Level.SEVERE, null, ex);
            ConexionBD.cerrarConexion(connection);
            throw new RuntimeException("Error al intentar aumentar la deuda de la tarjeta_credito "+ numeroTarjeta);
        }
    }
    /**
     ...
     */
    public boolean verificarLimite(long numeroTarjeta, double monto){
        ...
        try {
            ...
            statement.setLong(1, numeroTarjeta);
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
